...
| Workflow 引擎 | webhook、路由、重试、通知、转人工编排 | n8n 本地工作流已导入；风险判断和转人工仍在 `agent-service` |
| Agent 服务层 | 记忆、上下文、RAG、Tool、策略、审计编排 | 已有本地基础实现；生产级认证、调度和外部渠道能力待补齐 |
| Dify/模型层 | LLM 应用、Prompt、知识库、工具调用 | Dify 平台容器可运行；Chatflow 未发布，默认未配置模型供应商或 App Key |
| Tool 层 | AI 可调用的受控业务能力 | 已有 `db-simulator` |
| Adapter 层 | CRM/电商平台字段映射和 API 对接 | 待实现 |
| 数据层 | 业务库、Redis、Weaviate、审计 | 已有基础 |
| 监控评测层 | 质量、延迟、成本、命中率、安全评测 | 已有本地指标、审计与评测脚本；生产监控与告警待建设 |

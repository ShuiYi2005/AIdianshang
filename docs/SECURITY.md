...
当前项目已把 Compose 中的明文密码、密钥、API Key 改为变量引用，并通过 `deployment/.env` 注入。

已处理：

- `deployment/docker-compose.yml` 不再直接写死 `POSTGRES_PASSWORD`、`DIFY_SECRET_KEY`、`N8N_ENCRYPTION_KEY` 等敏感值。
- `deployment/.env.example` 只保留示例占位值。
- `.gitignore` 已忽略 `deployment/.env`、`dify/storage/` 和 `backups/`。

仍需注意：

- `deployment/.env` 当前仍保存本机可用的真实 Demo 密钥。
...
| 类型 | 当前位置 | 推荐位置 |
|---|---|---|
| Dify Secret Key | `deployment/.env` | KMS / Secret Manager |
| 数据库密码 | `deployment/.env` | KMS / Secret Manager |
| n8n Encryption Key | `deployment/.env` | KMS / Secret Manager |

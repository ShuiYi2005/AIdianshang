...
- `services/`：业务服务和 Tool 服务。当前包含 `db-simulator` 和 `agent-service`，后续增加 `api-gateway`、`crm-adapter`、`commerce-adapter`。
...
- Agent 服务层负责编排模型、记忆、上下文、RAG、Tool、策略与审计；当前 `agent-service` 已提供健康检查、脱敏预览、订单查询回复、context snapshot 和审计写入的最小纵切。

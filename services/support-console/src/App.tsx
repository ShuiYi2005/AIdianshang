...
export default function App({ api = apiClient }: { api?: ConsoleApi }) {
  ...
  const [workbenchModalOpen, setWorkbenchModalOpen] = useState(false);
  useEffect(() => {
    const targets = [document.querySelector(".global-header"), document.querySelector(".side-nav")];
    targets.forEach((target) => target?.toggleAttribute("inert", workbenchModalOpen));
  ...
  return <div className="app-frame">
    <header className="global-header" aria-hidden={workbenchModalOpen || undefined}><div className="brand-lockup"><span className="brand-icon"><StorefrontIcon weight="fill" /></span><div><strong>云栈店铺</strong><small>AI 客服运营台 · 模拟环境</small></div></div><div className="global-meta"><span><i className="live-dot" />模拟渠道在线</span><span>坐席：林墨</span><button className="icon-button" aria-label="通知" aria-expanded={notificationOpen} onClick={() => setNotificationOpen((open) => !open)}><BellIcon weight="duotone" /></button>{notificationOpen && <div className="notification-popover" role="status"><strong>模拟环境通知</strong><span>当前没有需要处理的系统通知。</span><button onClick={() => setNotificationOpen(false)}>知道了</button></div>}</div></header>
    <div className="app-body"><nav className="side-nav" aria-label="主导航" aria-hidden={workbenchModalOpen || undefined}><div className="nav-group">{navigation.map((item) => <button key={item.route} className={route === item.route ? "nav-item active" : "nav-item"} onClick={() => setRoute(item.route)} aria-current={route === item.route ? "page" : undefined}>{item.icon}<span>{item.label}</span></button>)}</div><div className="nav-footer"><ListBulletsIcon weight="duotone" /><span>所有外发均为本地模拟</span></div></nav><section className="app-content">{route === "workbench" && <Workbench api={api} onAuditChange={setAuditActions} onMobileModalChange={setWorkbenchModalOpen} />}{route === "training" && <TrainingCenter api={api} onAuditChange={setAuditActions} />}{route === "audit" && <AuditView actions={auditActions} />}</section></div>
  </div>;
}

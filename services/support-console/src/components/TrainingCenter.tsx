...
export function TrainingCenter({ api, onAuditChange }: { api: TrainingApi; onAuditChange?: (actions: TrainingTopicDetail["audit_actions"]) => void }) {
  ...
  return <main className="training-shell">
    <aside className="topic-list-panel"><div className="panel-heading"><div><span className="eyebrow">训练资产</span><h2>主题库</h2></div><button className="icon-button" onClick={newTopic} aria-label="新建主题快捷入口"><PlusIcon weight="bold" /></button></div><button className="new-topic-button" onClick={newTopic}><PlusIcon weight="bold" />新建主题</button><div className="topic-list">{topics.map((topic) => <button key={topic.id} className={topic.id === selectedId ? "topic-item selected" : "topic-item"} onClick={() => void loadDetail(topic.id)}><span><strong>{topic.name}</strong><small>{topic.trigger_phrases.slice(0, 2).join(" · ")}</small></span><em className={`topic-status ${topic.status}`}>{topic.status === "published" ? `V${topic.current_version}` : "草稿"}</em></button>)}{!topics.length && <p className="empty-state">尚无训练主题，创建第一条店铺经验。</p>}</div></aside>
    <section className="training-editor"><header className="page-heading"><div><span className="eyebrow">可验证训练</span><h1>AI 训练中心</h1><p>把店铺经验变成可验证、可回滚的 AI 回复。</p></div><div className="training-header-actions"><button className="secondary-action" disabled={!selectedId || busy === "preview"} onClick={() => void runPreview()}><PlayCircleIcon weight="fill" />快速预览</button><button className="primary-action" disabled={!selectedId || busy === "publish"} onClick={() => void publish()}><FloppyDiskIcon weight="fill" />发布训练</button></div></header>

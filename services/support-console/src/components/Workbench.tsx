...
export function Workbench({ api, onAuditChange, onMobileModalChange }: { api: WorkbenchApi; onAuditChange?: (actions: HandoffDetail["audit_actions"]) => void; onMobileModalChange?: (open: boolean) => void }) {
  ...
  const mobilePanelTrigger = useRef<HTMLElement | null>(null);
  ...
  useEffect(() => {
    ...
    void api.getHandoff(selectedId).then((next) => {
      if (active && requestVersion === detailRequestVersion.current) {
        setDetail(next);
  ...
  const refreshDetail = async (handoffId: string, fallback: HandoffDetail) => {
    const requestVersion = ++detailRequestVersion.current;
    try {
      const next = await api.getHandoff(handoffId);
      if (requestVersion !== detailRequestVersion.current) return;
      setDetail((current) => current?.handoff.id === handoffId ? next : current);
      onAuditChange?.(next.audit_actions);
    } catch {
      if (requestVersion !== detailRequestVersion.current) return;
      setDetail((current) => current?.handoff.id === handoffId ? fallback : current);
      setNotice("操作已完成；审计记录将在下次刷新时同步。");
    }
  };

  const closeMobilePanel = () => {
    setMobilePanel(null);
    window.requestAnimationFrame(() => mobilePanelTrigger.current?.focus());
  };

  const openMobilePanel = (panel: "queue" | "evidence") => {
    mobilePanelTrigger.current = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    setMobilePanel(panel);
  };

  useEffect(() => {
    if (!mobilePanel) return;
    const selector = mobilePanel === "queue" ? ".queue-panel .drawer-close" : ".evidence-panel .drawer-close";
    const frame = window.requestAnimationFrame(() => (document.querySelector(selector) as HTMLButtonElement | null)?.focus());
  ...
  useEffect(() => {
    const targets = [
      [document.querySelector(".queue-panel"), mobilePanel === "evidence"],
      [document.querySelector(".conversation-panel"), mobilePanel !== null],
      [document.querySelector(".evidence-panel"), mobilePanel === "queue"],
    ] as const;
    targets.forEach(([target, inert]) => target?.toggleAttribute("inert", inert));
    return () => targets.forEach(([target]) => target?.removeAttribute("inert"));
  }, [mobilePanel]);

  useEffect(() => {
    onMobileModalChange?.(mobilePanel !== null);
    return () => onMobileModalChange?.(false);
  }, [mobilePanel, onMobileModalChange]);
  ...
  const claim = async () => {
    ...
    try {
      ...
      const optimisticDetail = { ...detail, handoff: result.handoff };
      setDetail(optimisticDetail);
      setNotice("任务已领取，进入人工处理状态。");
      setQueueStatus("assigned");
      void refreshDetail(detail.handoff.id, optimisticDetail);
  ...
  const sendReply = async () => {
    ...
    try {
      ...
      const optimisticDetail = { ...detail, messages: [...detail.messages, result.message] };
      setDetail(optimisticDetail);
      setReply("");
      setNotice("模拟渠道已发送");
      void refreshDetail(detail.handoff.id, optimisticDetail);
  ...
  const createTicket = async () => {
    ...
    try {
      ...
      const optimisticDetail = { ...detail, tickets: [result.ticket, ...detail.tickets] };
      setDetail(optimisticDetail);
  ...
  const resolve = async () => {
    ...
    try {
      ...
      const optimisticDetail = { ...detail, handoff: { ...detail.handoff, status: "resolved" as const } };
      setDetail(optimisticDetail);
  ...
  return <main className="workbench-shell">
    {mobilePanel && <div className="drawer-backdrop" aria-hidden="true" onClick={closeMobilePanel} />}
    <section className={`queue-panel${mobilePanel === "queue" ? " is-mobile-open" : ""}`} role={mobilePanel === "queue" ? "dialog" : undefined} aria-hidden={mobilePanel === "evidence" || undefined} aria-modal={mobilePanel === "queue" || undefined} aria-label={mobilePanel === "queue" ? "会话列表" : "转人工队列"}>
      {mobilePanel === "queue" && <button className="drawer-close" aria-label="关闭会话列表" onClick={closeMobilePanel}>关闭</button>}
    ...
    <section className="conversation-panel" aria-hidden={mobilePanel !== null || undefined} aria-label="客服会话">
      {detail ? <>
        ...
        <div className="mobile-context-actions" aria-label="移动会话上下文"><button className="secondary-action queue-context-action" aria-expanded={mobilePanel === "queue"} onClick={() => openMobilePanel("queue")}>会话列表</button><button className="secondary-action" aria-expanded={mobilePanel === "evidence"} onClick={() => openMobilePanel("evidence")}>客户信息</button></div>
    ...
    <EvidencePanel detail={detail} mobileOpen={mobilePanel === "evidence"} backgroundHidden={mobilePanel === "queue"} onClose={closeMobilePanel} />
  </main>;
}
